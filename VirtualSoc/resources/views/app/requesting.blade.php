...
<div class="col-md-8 offset-md-2 mb-3">
    <div class="card">
        ...
        <div class="card-block">
            {{ Html::linkRoute("userPosts",$user->posts->count().' posts', array($user->id), array('class' => 'card-link')) }}
                    ...
                    <ul class="list-group list-group-flush">
                        @foreach ($user->friends as $friend)
                            <li class="list-group-item">
                                {{ Html::linkRoute("userProfile",$friend->name, array($friend->id), array()) }}
                            </li>
                        @endforeach
                    </ul>
                </div>
            </div>
            <form action = "{{route('requests.destroy')}}" method = "post">
                {{csrf_field()}}
                <input type="hidden" name="id" value={{$user->id}}>
                <button class="btn btn-danger pull-right" type="submit">Delete request</button>
            </form>
        </div>
    </div>
</div>
