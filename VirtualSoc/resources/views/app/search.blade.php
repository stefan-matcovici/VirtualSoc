    ...
    <div class="card">
        ...
        <div class="card-block">
            ...
            @if (Auth::user()->friends->contains($user->id))
                <a href="#" class="btn btn-warning pull-right disabled" aria-disabled="true">Friends</a>
            @elseif (!Auth::user()->requesting->contains($user->id))
                <form action = "{{route('requests.store')}}" method = "post">
                    {{csrf_field()}}
                    <input type="hidden" name="id" value={{$user->id}}>
                    <button class="btn btn-primary pull-right" type="submit">Send request</button>
                </form>
            @else
                <a href="#" class="btn btn-primary pull-right disabled" aria-disabled="true">Request sent</a>
            @endif
        </div>
    </div>
</div>
